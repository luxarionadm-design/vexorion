...
export class useHash {
  ...
  // Lightweight MD5 implementation for standard browser compatibility
  md5(text) {
  ...
  base64(text) { return btoa(unescape(encodeURIComponent(text))); }
  base64Decode(base64) { return decodeURIComponent(escape(atob(base64))); }
  urlSafeBase64(text) { return this.base64(text).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, ''); }
}
